 ...
 * persisted history. The block is generated by
 * `buildMentionContextBlock`; this parser is the inverse used to
 * re-render the chips in the user message bubble on reload. Lines that
 * don't match `- <type>: <path>` are skipped so a hand-edited
 * transcript can't crash the renderer.
 *
 * The trailing separator (`\n` or `\n\n` between the block and the
 * next paragraph — typically the skill marker chain) is consumed so
 * the rest of the pipeline (`parseSkillMarkers`, the
 * `CollapsibleUserMessage` markdown view) sees a clean, unindented
 * payload. Without this, the second parser would have to skip a
 * leading newline before its leading-anchored marker regex matched.
 */
export function parseMentionBlock(rawText: string): {
  ...
  // Drop any leading newlines the block left behind so the
  // remaining text starts with the user payload (or the skill
  // marker chain, which `parseSkillMarkers` anchors with `^`).
  const rest = rawText.slice(match[0].length).replace(/^\n+/, "");
  return { mentions, text: rest };
}
